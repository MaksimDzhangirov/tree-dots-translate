...
func simulateEvents(db *stdSQL.DB) {
	for {
		...
		// В настоящем приложении, это место, где должен сохраняться какой-то агрегат,
		// используя ту же транзакцию.
		// tx.Exec("INSERT INTO (...)")
...
// publishEvent публикует новое событие.
// Чтобы опубликовать событие в отдельной транзакции, нужно создавать каждый раз новый SQL Publisher
// передавая соответствующий обработчик транзакции.
