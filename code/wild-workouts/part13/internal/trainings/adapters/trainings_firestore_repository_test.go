...
import (
	...
	"github.com/MaksimDzhangirov/three-dots/part13/internal/trainings/adapters"
	"github.com/MaksimDzhangirov/three-dots/part13/internal/trainings/app/query"
...
// todo - сделать тесты параллельными после исправления ошибки в эмуляторе: https://github.com/firebase/firebase-tools/issues/2452
...
func TestTrainingsFirestoreRepository_AllTrainings(t *testing.T) {
	...
	// AllTrainings возвращает все документы, из-за этого нам необходимо в качестве исключения осуществить очистку базы
	// В общем случае, я рекомендую делать её перед тестом. Таким образом, вы будете уверены, что очистка сделана.
	// Благодаря этому тесты более стабильные.
	// Подробнее о том, почему это важно, вы можете узнать в https://threedots.tech/post/database-integration-testing/
...
// Firestore не хранит время с такой же точностью, поэтому нам нужно немного округлить его
