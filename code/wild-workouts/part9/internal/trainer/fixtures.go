...
import (
	...
	"github.com/MaksimDzhangirov/three-dots/part9/internal/common/client"
...
type fixturesChecker interface {
	CanLoadFixtures(ctx context.Context, dayToSet int) (bool, error)
}
...
func loadFixtures(checker fixturesChecker) {
	...
	for {
		...
		logrus.WithError(err).Errorf("Cannot check if fixtures can be loaded %v", err)
