...
import (
	...
	orders_http_interface "github.com/MaksimDzhangirov/three-dots/code/monolith-microservice-shop/pkg/orders/interfaces/public/http"
...
var testCases = []struct {
	...
	{
		Name:                 "monolith",
		OrdersServiceAddress: "http://monolith:8080", // тесты запускаются из контейнера, поэтому :8080, а не :8090
		ShopServiceAddress:   "http://monolith:8080",
	},
	{
		Name:                 "microservices",
		OrdersServiceAddress: "http://orders:8080", // тесты запускаются из контейнера, поэтому :8080, а не :8070
...
func placeOrder(t *testing.T, ordersServiceAddress string, productID string) string {
	resp := makeRequest(t, "POST", ordersServiceAddress+"/orders", orders_http_interface.PostOrderRequest{
...
func makeRequest(t *testing.T, method string, path string, data interface{}) *http.Response {
	...
	// resp.Body - это io.Reader и считается потоком, поэтому вы можете считать из него единожды.
	// Мы должны присвоить значение Body опять, чтобы опять его прочитать.
